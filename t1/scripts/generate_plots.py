...
def find_result_files(base_dir, dataset_name):
    ...
    # Find Spark results for the specified dataset
    spark_patterns = [
        f"{dataset_name}/spark/final-output-*/part-00000",
        f"{dataset_name}/spark/output-*/part-00000", 
        f"{dataset_name}/spark/*/part-00000",
        f"spark/{dataset_name}/output-*/part-00000"
    ]
    ...
    for pattern in spark_patterns:
        spark_files.extend(glob.glob(str(base_path / pattern)))
    
    return hadoop_files, spark_files

def parse_distribution_file(file_path):
    """Parse distribution data from either Hadoop or Spark output"""
    degrees = []
    counts = []
    ...
    try:
        with open(file_path, 'r') as f:
            for line in f:
                ...
                try:
                    # Handle Spark format: (degree, count)
                    if line.startswith('(') and line.endswith(')'):
                        ...
                        # Handle Hadoop format: degree\tcount or degree count
...
def create_comparison_plots(hadoop_data, spark_data, dataset_name, output_dir="./"):
    ...
    if has_spark:
        ax1.scatter(s_deg, s_cnt, alpha=0.7, s=30, color='red', label='Apache Spark', edgecolors='darkred', marker='x')
    ...
    if has_spark:
        ...
        ax2.loglog(s_deg[s_pos], s_cnt[s_pos], 'x', alpha=0.7, markersize=6, color='red', label='Spark')
    ...
    if has_spark:
        ax3.semilogy(s_deg, s_cnt, 'x-', alpha=0.7, markersize=4, color='red', label='Spark')
...
def main():
    ...
    # Find result files
    hadoop_files, spark_files = find_result_files(results_dir, dataset_name)
    
    print(f"\nFound files:")
    print(f"   Hadoop files: {len(hadoop_files)}")
    for f in hadoop_files:
        print(f"     - {f}")
    print(f"   Spark files: {len(spark_files)}")
    for f in spark_files:
        print(f"     - {f}")
    
    if not hadoop_files and not spark_files:
        print(f"No result files found for dataset '{dataset_name}'!")
        print(f"Expected locations:")
        print(f"   - {results_dir}/{dataset_name}/hadoop/output-*/part-r-00000")
        print(f"   - {results_dir}/{dataset_name}/spark/final-output-*/part-00000")
        return
    
    # Use the most recent files
    hadoop_file = hadoop_files[-1] if hadoop_files else None
    spark_file = spark_files[-1] if spark_files else None
    ...
    if hadoop_file:
        ...
        hadoop_data = parse_distribution_file(hadoop_file)
    
    if spark_file:
        print(f"   Spark: {spark_file}")
        spark_data = parse_distribution_file(spark_file)
