...
configure_kafka_persistence() {
    echo " Configuring Kafka for persistent storage..."
...
configure_zookeeper_persistence() {
    echo " Configuring Zookeeper for persistent storage..."
...
main_setup() {
    ...
    if pgrep -f "kafka.Kafka\|QuorumPeerMain" > /dev/null; then
        echo "  Kafka/Zookeeper is running. Stopping for configuration..."
